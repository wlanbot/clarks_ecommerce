...
class MercadoPagoAdapter {
  ...
  async createPayment(options) {
    const callbackUrl = options.callbackUrl || process.env.PAYMENT_CALLBACK_URL;
    ...
    const preferenceData = {
      ...
      external_reference: options.metadata?.orderId,
      auto_return: 'approved',
      notification_url: MERCADO_PAGO_WEBHOOK_URL,
      statement_descriptor: PAYMENT_STATEMENT_DESCRIPTOR || 'Mi Aplicación',
    };
    ...
    try {
      ...
      this.logger.error('MercadoPago create payment error:', error);
  ...
  async refundPayment(options) {
    try {
      ...
      return {
        id: orderId,
        status: this.mapStatus(responseData.status), // 'APPROVED'
      ...
      if (error.response) {
        ...
        this.logger.error('MercadoPago refund error:', responseData);
  ...
  async getRefunds(paymentIds) {
    try {
      ...
      if (error.response) {
        const responseData = error.response.data;
        this.logger.error('MercadoPago get refunds error:', responseData);
        throw new PaymentProviderError(
          'MercadoPago',
          new Error(responseData.message || 'Failed to get refunds')
        );
      } else {
        this.logger.error('MercadoPago get refunds error:', error);
        throw new PaymentProviderError('MercadoPago', error);
      }
    }
  }
  ...
  async getRefund(paymentIds) {
    ...
    try {
      ...
      if (error.response) {
        const responseData = error.response.data;
        this.logger.error('MercadoPago get refund error:', responseData);

        if (error.response.status === 404) {
          throw new Error(`Refund ${refundId} not found for payment ${transactionId}`);
        }
        ...
        this.logger.error('MercadoPago get refund error:', error);
        throw new PaymentProviderError('MercadoPago', error);
      }
    }
  }
  ...
  async processWebhook(payload) {
    try {
      ...
      // Determine payload type and extract payment ID
      let paymentId = null;
      ...
      if (action && data && data.id) {
        if (action.startsWith('payment.')) {
          ...
          this.logger.log(`Detected v1 payment webhook, ID: ${paymentId}`);
        }
      }
      // Case 2: v0 API format (resource + topic=payment)
        ...
        paymentId = resourceStr.includes('/')
          ...
          : resourceStr;
        isPaymentNotification = true;
        this.logger.log(`Detected v0 payment webhook, ID: ${paymentId}`);
      }
      // Case 3: Merchant order notification
      else if (topic === 'merchant_order' && resource) {
        const merchantOrderUrl = resource;
        this.logger.log(`Detected merchant order webhook: ${merchantOrderUrl}`);
        ...
        try {
          ...
          if (
            ...
            this.logger.warn(
              `Merchant order ${merchantOrderId} has no payments`,
            );
            return null;
          }
        } catch (error) {
      ...
      this.logger.warn('Unrecognized webhook format');
      return null;
    } catch (error) {
      this.logger.error('MercadoPago webhook processing error:', error);
      throw new PaymentProviderError('MercadoPago', error);
    }
  }
  ...
  async getPaymentByTransactionId(transactionId) {
    this.logger.log(`Fetching payment with transactionId: ${transactionId}`);
    try {
  ...
  async searchPaymentByOrderId(orderId) {
    this.logger.log(`Searching payment with orderId: ${orderId}`);
    ...
    try {
      ...
      this.logger.log(
        `Search results count: ${searchResult.data.results?.length || 0}`,
      );

      if (!searchResult.data.results?.length) {
        this.logger.warn(`No payments found for orderId: ${orderId}`);
        return 'PENDING';
      }

      const payment = searchResult.data.results[0];
      this.logger.log(
        `Found payment ID: ${payment.id}, status: ${payment.status}`,
      );

      return this.mapStatus(payment.status);
    } catch (error) {
      this.logger.error('Error searching payment:', error);
      return 'PENDING';
    }
  }
  ...
  async processPaymentUpdate(paymentId) {
    try {
      ...
      if (!payment) {
        this.logger.warn(
          `Payment with ID ${paymentId} not found in Mercado Pago`,
        );
        return null;
      }
      ...
      this.logger.error(
        `Error fetching payment ${paymentId} from Mercado Pago:`,
        error,
      );
      return null;
    }
  ...
  async fetchMerchantOrder(merchantOrderId) {
    try {
      ...
      this.logger.error(
        `Unexpected error fetching merchant order ${merchantOrderId}:`,
        error,
      );
      throw new PaymentProviderError(
        'MercadoPago',
        new Error(
          `Error fetching merchant order ${merchantOrderId}: ${error.message}`,
        ),
      );
    }
  }
...
module.exports = new MercadoPagoAdapter();